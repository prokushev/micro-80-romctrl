...
; ───────────────────────────────────────────────────────────────────────
	...
	LD	((RST0_2-$) & 0ffffh), A
; ───────────────────────────────────────────────────────────────────────
; Переключаем на свой стек
; ───────────────────────────────────────────────────────────────────────
	LD	HL, 0			; Получаем текущее значение SP
	ADD	HL, SP
	EX	DE, HL
	RST	0
	LD	HL, (Patch4+1)-$	; Сохраням для последующего восстановления
	LD	(HL), E
	INC	HL
	LD	(HL), D
	RST	0			; Ставим свой стек
Stack:
T:	LD	SP, Stack-$		; также будет буфер DB 0FH DUP 0
; ───────────────────────────────────────────────────────────────────────
...
SEARCH:	PUSH	BC
	...
	PUSH	BC
Patch1:
	CALL	ReadROM		; HL - начало, DE - конец, BC - посадка
	POP	BC
	INC	DE
	LD	A,(BC)
...
; Восстанавливаем память 0-2
	...
	RST	0
	LD	SP, (T+8)-$	; Используя стек, ниже читаем...
...
Patch4:
	LD	SP, 0		; Восстанавливаем первоначальный стек
